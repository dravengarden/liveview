...
// ║  Source of truth: columbus/interface/app-shell/nav-shell.tsx
// ║  Edit there, then re-run `harness shell sync` to propagate.      ║
// ╚════════════════════════════════════════════════════════════════╝

// NavShell — the unified app-navigation primitive for the atlantis ecosystem.
//
...
//     menu toggle, the title, app actions, and the launcher trigger.
...
import { PortalLauncherButton } from "./app-grid-button";
...
// and the atlantis portal iframe (which eats horizontal room) should all get
// the drawer rather than a cramped desktop sidebar.
...
export interface NavShellProps {
  ...
  /** App-specific top-bar actions (e.g. settings), placed before the launcher. */
...
export function NavShell(props: NavShellProps): ReactNode {
  ...
  // it can't linger as a stuck overlay over the now-permanent sidebar.
  useEffect(() => {
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "100dvh", overflow: "hidden" }}>
      ...
      >
        <Tooltip title={sidebarShown ? "Collapse" : "Menu"}>
          <IconButton aria-label="toggle navigation" onClick={onToggle} size="small">
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, flexShrink: 0 }}>
          {actions}
          {/* Launcher trigger self-hides when the app isn't portal-hosted. */}
