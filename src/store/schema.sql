...
CREATE TABLE IF NOT EXISTS books (
    ...
    -- changed. 0 until first stamped (see PgStore::mark_book). NOT git times.
    created_at        BIGINT NOT NULL DEFAULT 0,
    updated_at        BIGINT NOT NULL DEFAULT 0
);
-- Backfill the timestamp columns onto pre-existing deploys (CREATE TABLE IF NOT
...
--
-- Audio (audiobook .spoken.md + text read-aloud) is generated by a background
-- worker INSIDE the running server, not inline in `liveview sync` (which now
...
CREATE TABLE IF NOT EXISTS audio_tasks (
    ...
    content_hash TEXT    NOT NULL,          -- chapter source hash this task is for
    leaf_kind    TEXT    NOT NULL,          -- Merkle leaf kind (to commit the node)
    voice        TEXT    NOT NULL,
    status       TEXT    NOT NULL DEFAULT 'queued',  -- queued|running|done|failed
    priority     INTEGER NOT NULL DEFAULT 0,         -- higher first (interactive=100)
...
-- Worker claim order: highest priority, oldest first, among runnable rows.
CREATE INDEX IF NOT EXISTS audio_tasks_claim
    ON audio_tasks (priority DESC, enqueued_at ASC)
    WHERE status = 'queued';
