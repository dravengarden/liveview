...
//!
//! `allow(dead_code)`: the IR is the **wire contract** the web renderer consumes.
//! Many fields (chart `mark`/`overlays`, widget `min`/`max`/`step`, `format`,
//! `audio`, …) are deserialized and carried to the client but not read by *this*
//! binary yet — they are load-bearing for the renderer, not dead.
#![allow(dead_code)]
